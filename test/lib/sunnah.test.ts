...
import CalculationMethod from "../../src/lib/CalculationMethod";
import Coordinates from "../../src/lib/Coordinates";
import HighLatitudeRule from "../../src/lib/HighLatitudeRule";
import PrayerTimes from "../../src/lib/PrayerTimes";
import SunnahTimes from "../../src/lib/SunnahTimes";
...
test("getting sunnah times for the New York timezone", () => {
  ...
  ).toBe("7/13/15, 12:38 AM");
  expect(
    moment(sunnah.lastThirdOfTheNight)
      .tz("America/New_York")
      .format("M/D/YY, h:mm A")
  ).toBe("7/13/15, 1:59 AM");
});
...
test("getting sunnah times for the London timezone", () => {
  ...
  const sunnah = new SunnahTimes(p1);
  expect(
    moment(sunnah.middleOfTheNight).tz("Europe/London").format("M/D/YY, h:mm A")
  ).toBe("12/31/16, 11:15 PM");
  expect(
    moment(sunnah.lastThirdOfTheNight)
      .tz("Europe/London")
      .format("M/D/YY, h:mm A")
...
test("getting sunnah times for US DST change", () => {
  ...
  expect(
    moment(p1.maghrib).tz("America/Los_Angeles").format("M/D/YY, h:mm A")
  ...
  expect(
    moment(sunnah.middleOfTheNight)
      ...
      .format("M/D/YY, h:mm A")
  ).toBe("3/11/17, 11:43 PM");
  expect(
    moment(sunnah.lastThirdOfTheNight)
      .tz("America/Los_Angeles")
      .format("M/D/YY, h:mm A")
  ).toBe("3/12/17, 1:33 AM");
});
...
test("getting sunnah times for Europe DST change", () => {
  ...
  expect(moment(p1.maghrib).tz("Europe/Paris").format("M/D/YY, h:mm A")).toBe(
    "10/24/15, 6:45 PM"
  );
  ...
  expect(moment(p2.maghrib).tz("Europe/Paris").format("M/D/YY, h:mm A")).toBe(
    "10/25/15, 5:43 PM"
  );
